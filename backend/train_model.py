...
def train_and_evaluate():
    ...
    # --- Phase 1: Fit preprocessor on raw training data ---
    # SMOTE is applied inside the ImbPipeline during cross-validation so that synthetic samples are never seen by the validation fold.
    print("\n--- Phase 1: Preprocessing ---")
    X_train_processed = preprocessor.fit_transform(X_train)
    X_test_processed  = preprocessor.transform(X_test)

    # --- Phase 2: Hyperparameter Tuning ---
    # Each model is wrapped in an ImbPipeline so SMOTE is applied only to the training fold during cross-validation.
    ...
    smote = SMOTE(random_state=29, sampling_strategy='auto')

    # Model configs with hyperparameter search spaces.
    model_configs = {
        'Logistic Regression': {
            'model': LogisticRegression(max_iter=2000, random_state=29),
            'params': {
        ...
        'XGBoost': {
            'model': XGBClassifier(eval_metric='logloss', random_state=29, verbosity=0),
            'params': {
                'classifier__n_estimators': [100, 200, 300, 500],
    ...
    print("\n" + "=" * 60)
    print("MODEL TRAINING & EVALUATION (with SMOTE and hypertuning)")
    print("=" * 60)
    ...
    for name, config in model_configs.items():
        ...
        # Wraping classifier and SMOTE into a single pipeline so smote only sees each cv training fold.
        ...
        search = RandomizedSearchCV(
            ...
            random_state=29, n_jobs=n_jobs, verbose=0
        )
    ...
    # preprocessor (sklearn) - best ImbPipeline (smote and classifier)
    # At inference time smote is skipped automatically because predict() is called, not fit_resample(). The serving pipeline only uses the trained classifier.
